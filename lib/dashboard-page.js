...
function applyWorkspaceRoleUi() {
  ...
  if (viewerMode) {
    const cabang = sessionStorage.getItem('viewerCabang') || '';
...
function clearWorkspaceSessions() {
  ...
  sessionStorage.removeItem('viewerAuthTs');
  sessionStorage.removeItem('viewerCabang');
}
...
async function boot() {
  ...
  // Viewer: login via #viewer hash + sessionStorage
  if (!activePrefix && location.hash === '#viewer') {
    location.replace('/');
    return;
  }
