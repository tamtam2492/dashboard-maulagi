...
function getWorkspaceSnapshotStatus(periode, syncInfo) {
  ...
  if (!syncInfo) return baseLabel + ' · Status snapshot belum tersedia';
  if (syncInfo.manualMode) {
    const syncStamp = getSyncStampLabel(syncInfo);
    if (syncInfo.error) return baseLabel + ' · Status upload manual bermasalah';
    if (syncStamp) return baseLabel + ' · Snapshot upload manual ' + syncStamp;
    return baseLabel + ' · Mode upload manual aktif';
  }
  if (syncInfo.error) return baseLabel + ' · Status snapshot bermasalah';
  ...
  if (syncInfo.refreshState && syncInfo.refreshState.status === 'blocked') {
    return baseLabel + ' · Snapshot perlu refresh, ' + getNoncodWorkerStateLabel(syncInfo);
  }

  const syncStamp = getSyncStampLabel(syncInfo);
  if (syncStamp) return baseLabel + ' · Snapshot terakhir terbit ' + syncStamp;
  ...
  const pipelineStamp = syncInfo.pipeline && syncInfo.pipeline.lastPublishedAt
    ...
    : '';
  if (pipelineStamp) return baseLabel + ' · Publish worker terakhir ' + pipelineStamp;

  if (!syncInfo.enabled) return baseLabel + ' · Auto-refresh snapshot belum aktif';
...
function updateWorkspaceSyncBanner(syncInfo) {
  ...
  if (!syncInfo) {
    ...
    setMainStatus('bi-arrow-repeat', 'Status snapshot belum tersedia');
    sub.style.display = '';
  ...
  if (syncInfo.manualMode) {
    banner.dataset.tone = syncStamp ? 'ok' : 'warn';
    setMainStatus('bi-upload', 'Mode upload manual aktif');
  ...
  if (syncInfo.error) {
    ...
    setMainStatus('bi-exclamation-circle', 'Refresh snapshot MauKirim gagal, memakai data terakhir');
    sub.textContent = statsText;
    return;
  }
  if (!syncInfo.enabled) {
    banner.dataset.tone = 'warn';
    setMainStatus('bi-pause-circle', 'Auto-refresh snapshot MauKirim belum aktif');
  ...
  if (syncInfo.refreshState && syncInfo.refreshState.status === 'blocked') {
    banner.dataset.tone = 'error';
    setMainStatus('bi-exclamation-triangle', 'Snapshot perlu refresh, ' + getNoncodWorkerStateLabel(syncInfo));
    sub.textContent = (syncStamp ? 'Snapshot terakhir ' + syncStamp + ' · ' : '') + statsText;
    return;
  }
  ...
  if (syncInfo.performed) {
    setMainStatus('bi-arrow-clockwise', 'Snapshot baru diterbitkan dari MauKirim');
    sub.textContent = (syncStamp ? 'Publish snapshot ' + syncStamp + ' · ' : '') + statsText;
    return;
  }

  setMainStatus('bi-arrow-repeat', syncStamp ? 'Snapshot terakhir ' + syncStamp : 'Snapshot tersimpan');
  sub.textContent = '';
