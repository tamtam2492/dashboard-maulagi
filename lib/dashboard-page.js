...
function hasActiveSession(prefix) {
  ...
  return !!sessionStorage.getItem(prefix + 'Auth') && ts && (Date.now() - ts <= SESSION_TIMEOUT);
}

async function hasWorkspaceServerSession(prefix = getActiveSessionPrefix()) {
  const role = prefix === 'admin' ? 'admin' : 'dashboard';
  try {
...
function setSessionTs(prefix = getActiveSessionPrefix()) {
  if (!prefix) return;
  sessionStorage.setItem(prefix + 'AuthTs', String(Date.now()));
}
...
function applyWorkspaceRoleUi() {
  ...
  document.getElementById('shellUploadBtn').style.display = adminMode ? '' : 'none';
...
function clearWorkspaceSessions() {
  ...
  sessionStorage.removeItem('dashAuthTs');
  sessionStorage.removeItem('dashToken');
  sessionStorage.removeItem('adminAuth');
  sessionStorage.removeItem('adminAuthTs');
...
function updateWorkspaceSyncBanner(syncInfo) {
  ...
  setMainStatus(syncInfo.performed ? 'bi-arrow-clockwise' : 'bi-arrow-repeat', syncInfo.performed
    ? 'Baru sinkron dari MauKirim'
    : 'Data MauKirim tersimpan');
  sub.textContent = (syncStamp ? (syncInfo.performed ? 'Waktu sync ' : 'Terakhir sync ') + syncStamp + ' · ' : '') + statsText;
}
...
function setLatestTransferChip(detail) {
  ...
  if (!chip) return;
  if (!detail) {
    chip.textContent = 'Belum ada transfer hari ini';
    chip.disabled = true;
...
async function refreshEmbeddedFrame(frameId) {
  const frame = document.getElementById(frameId);
...
async function requestWorkspaceRefresh(options = {}) {
  ...
  workspaceRefreshPromise = (async () => {
    ...
    if (includeFrames) {
      const frameTasks = getLoadedWorkspaceFrameIds().map(frameId => refreshEmbeddedFrame(frameId));
...
async function loadOverview(forceSpin) {
  ...
  try {
    ...
    document.getElementById('statusChip').textContent = 'Perlu cek';
...
async function boot() {
  let activePrefix = getActiveSessionPrefix();
  ...
  refreshTimer = setInterval(() => {
    requestWorkspaceRefresh({
      source: 'workspace_timer',
      spinOverview: false,
      includeAudit: shouldRefreshAuditWorkspace(),
      includeFrames: true,
    });
  }, 180000);
}
