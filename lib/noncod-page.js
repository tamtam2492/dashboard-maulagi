...
let cabangAreaMap = {};
let allAreas = [];
const DEFAULT_NONCOD_STATUS = 'semua';
...
function initNoncodEventBindings() {
  ...
  const uploadBtn = document.getElementById('btnUploadWorkbook');
  const downloadBtn = document.getElementById('btnDownload');
  const uploadInput = document.getElementById('noncodUploadInput');
  ...
  if (uploadBtn) uploadBtn.addEventListener('click', () => { toggleUpload(); });
  if (downloadBtn) downloadBtn.addEventListener('click', downloadXlsx);
  if (uploadInput) uploadInput.addEventListener('change', handleManualUploadSelection);
...
function canManualUploadWorkbook() {
  return getActiveSessionPrefix() === 'admin';
}

function syncManualUploadUi() {
  const uploadBtn = document.getElementById('btnUploadWorkbook');
  if (!uploadBtn) return;

  uploadBtn.style.display = canManualUploadWorkbook() && !EMBED_MODE ? '' : 'none';
  uploadBtn.classList.toggle('active', manualUploadBusy);
  uploadBtn.style.pointerEvents = manualUploadBusy ? 'none' : '';
  uploadBtn.style.opacity = manualUploadBusy ? '0.75' : '';
  uploadBtn.title = manualUploadBusy
    ? 'Mengunggah workbook manual...'
    : 'Upload workbook XLSX MauKirim manual';
}

function toggleUpload() {
  if (!canManualUploadWorkbook()) {
    showToast('Hanya admin yang bisa upload workbook manual.', 'error');
    return false;
  }
  if (manualUploadBusy) return false;
...
async function handleManualUploadSelection(event) {
  const input = event && event.target;
  const file = input && input.files && input.files[0] ? input.files[0] : null;
  try {
    if (file) await uploadManualWorkbook(file);
  } finally {
    if (input) input.value = '';
  }
}

async function uploadManualWorkbook(file) {
  if (!canManualUploadWorkbook()) {
    showToast('Hanya admin yang bisa upload workbook manual.', 'error');
    return false;
  }
  if (!isWorkbookXlsx(file)) {
    showToast('File harus berupa workbook XLSX dari MauKirim.', 'error');
    return false;
  }

  const periode = getSelectedPeriode();
  if (!periode) {
    showToast('Pilih periode dulu sebelum upload workbook.', 'error');
    return false;
  }

  manualUploadBusy = true;
  syncManualUploadUi();
  ...
  try {
    ...
    formData.append('workbook', file, file.name || 'noncod-manual.xlsx');

    const res = await fetch('/api/noncod', {
      method: 'POST',
      body: formData,
    ...
    if (!res.ok) {
      throw new Error(json && json.error ? json.error : 'Gagal upload workbook manual.');
    }

    lastSyncInfo = json && json.syncInfo ? json.syncInfo : lastSyncInfo;
    showToast(
      json && json.reusedSnapshot
        ? 'Workbook sama dengan snapshot tersimpan. Data tidak berubah.'
        : 'Workbook manual berhasil diimpor.',
      'success'
    );
    await loadDashboard({ forceRefresh: true, silent: true });
    return true;
  } catch (err) {
    showToast(err && err.message ? err.message : 'Gagal upload workbook manual.', 'error');
    return false;
  } finally {
    manualUploadBusy = false;
    syncManualUploadUi();
  }
}
...
function getNoncodWorkerStateLabel(syncInfo) {
...
function formatSyncLabel(syncInfo) {
  if (!syncInfo) return ' · Status snapshot belum diketahui';
  let stamp = '';
  ...
  if (syncInfo.manualMode) {
    if (syncInfo.error) return ' · Snapshot upload manual bermasalah, pakai data terakhir';
    if (stamp) return ' · Snapshot upload manual' + stamp;
    return ' · Mode upload manual aktif';
  }
  if (syncInfo.error) return ' · Refresh snapshot MauKirim gagal, pakai data terakhir';
  if (!syncInfo.enabled) return ' · Auto-refresh snapshot MauKirim belum aktif';
  if (!syncInfo.eligible) return ' · Periode ini pakai snapshot tersimpan';
  if (syncInfo.refreshState && syncInfo.refreshState.status === 'running' && syncInfo.refreshState.action !== 'inline') {
    return ' · Snapshot sedang disegarkan otomatis' + (stamp ? ' · snapshot terakhir' + stamp : '');
  }
  if (syncInfo.refreshState && syncInfo.refreshState.status === 'queued') {
    return ' · Snapshot dijadwalkan refresh otomatis' + (stamp ? ' · snapshot terakhir' + stamp : '');
  }
  if (syncInfo.refreshState && syncInfo.refreshState.status === 'blocked') {
    return ' · Snapshot perlu refresh, ' + getNoncodWorkerStateLabel(syncInfo) + (stamp ? ' · snapshot terakhir' + stamp : '');
  }
  if (syncInfo.source === 'manual_upload') return ' · Data tersimpan sebelumnya' + (stamp ? ' · snapshot terakhir' + stamp : '');
  if (syncInfo.performed) return ' · Snapshot baru diterbitkan dari MauKirim' + (stamp ? ' · publish terakhir' + stamp : '');
  return ' · Snapshot MauKirim tersimpan' + (stamp ? ' · snapshot terakhir' + stamp : '');
}
