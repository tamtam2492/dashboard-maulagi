const EMBED_MODE = new URLSearchParams(location.search).get('embed') === '1' || window.self !== window.top;
      ...
      const ADMIN_SNAPSHOT_REFRESH_MS = 60000;
      ...
      function startAdminSnapshotLoop() {
        if (adminUiState.snapshotLoopId) return;
        adminUiState.snapshotLoopId = window.setInterval(async () => {
          if (!adminUiState.bootReady || document.hidden) return;
          await loadMismatchMonitor({ forceRefresh: true, silent: true });

          const auditModal = document.getElementById('modalAuditCabang');
          if (auditModal && auditModal.classList.contains('show')) {
            await loadAuditCabangModal(true, { silent: true });
          }
      ...
      async function refreshAdminViewsAfterWrite(options = {}) {
        ...
        await loadMismatchMonitor({ forceRefresh: true, silent: true });
        refreshAuditCabangIfOpen({ forceRefresh: true, silent: true });
      }

      window.workspaceRefreshFromParent = async function() {
        await loadMismatchMonitor({ forceRefresh: true, silent: true });
        const auditModal = document.getElementById('modalAuditCabang');
        if (auditModal && auditModal.classList.contains('show')) {
          await loadAuditCabangModal(true, { silent: true });
      ...
      async function addCabang() {
        ...
        btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Menyimpan...';
        try {
          const res = await fetch('/api/cabang', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Admin-Token': getOpsToken() }, body: JSON.stringify({ nama: val, area: document.getElementById('inputArea').value }) });
          const json = await res.json();
          if (!res.ok) { showToast(json.error || 'Gagal menambah cabang', 'error'); }
          else { input.value = ''; document.getElementById('inputArea').value = ''; showToast('Cabang berhasil ditambahkan', 'success'); await loadCabangModal({ forceRefresh: true, silent: true }); }
      ...
      async function deleteCabang(id, name) {
        ...
        try {
          ...
          else { showToast('Cabang dihapus', 'success'); await loadCabangModal({ forceRefresh: true, silent: true }); }
        } catch { showToast('Terjadi kesalahan jaringan', 'error'); }
      }
      ...
      async function saveEditCabang(id) {
        ...
        try {
          const res = await fetch('/api/cabang', { method: 'PUT', headers: { 'Content-Type': 'application/json', 'X-Admin-Token': getOpsToken() }, body: JSON.stringify({ id, nama, area }) });
          const json = await res.json();
          if (!res.ok) { showToast(json.error || 'Gagal mengubah cabang', 'error'); }
          else { showToast('Cabang diperbarui', 'success'); await loadCabangModal({ forceRefresh: true, silent: true }); }
        } catch { showToast('Terjadi kesalahan jaringan', 'error'); }
      }
