const EMBED_MODE = new URLSearchParams(location.search).get('embed') === '1' || window.self !== window.top;
      ...
      let cabangAutoSyncPromise = null;
      ...
      function formatAdminSyncStamp(syncInfo) {
      ...
      function getAdminNoncodWorkerStateLabel(syncInfo) {
        return syncInfo && syncInfo.triggerMode === 'invalid'
          ? 'worker NONCOD belum valid'
          : 'worker NONCOD belum aktif';
      }

      function getAdminSnapshotStateLabel(syncInfo) {
        if (!syncInfo) return 'status snapshot belum tersedia';

        const snapshotStamp = formatAdminSyncStamp(syncInfo);
        const snapshotSuffix = snapshotStamp ? ' · snapshot terakhir ' + snapshotStamp : '';
        if (syncInfo.manualMode) {
          if (syncInfo.error) return 'snapshot upload manual bermasalah, memakai data terakhir';
          if (snapshotStamp) return 'snapshot upload manual ' + snapshotStamp;
          return 'mode upload manual aktif';
        }
        if (syncInfo.error) return 'refresh snapshot bermasalah, memakai data terakhir';
        if (!syncInfo.enabled) return 'auto-refresh snapshot belum aktif';
        if (!syncInfo.eligible) return 'periode memakai snapshot tersimpan';
        if (syncInfo.refreshState && syncInfo.refreshState.status === 'running' && syncInfo.refreshState.action !== 'inline') {
          return 'snapshot sedang disegarkan worker' + snapshotSuffix;
        }
        if (syncInfo.refreshState && syncInfo.refreshState.status === 'queued') {
          return 'snapshot dijadwalkan ke worker' + snapshotSuffix;
        }
        if (syncInfo.refreshState && syncInfo.refreshState.status === 'blocked') {
          return 'snapshot perlu refresh, ' + getAdminNoncodWorkerStateLabel(syncInfo) + snapshotSuffix;
        }
        if (snapshotStamp) return 'snapshot terakhir terbit ' + snapshotStamp;

        const publishedStamp = syncInfo.pipeline && syncInfo.pipeline.lastPublishedAt
          ? formatAdminSyncStamp({ syncedAt: syncInfo.pipeline.lastPublishedAt })
          : '';
        if (publishedStamp) return 'publish worker terakhir ' + publishedStamp;
      ...
      async function handleAdminDelegatedClick(event) {
        ...
        switch (action) {
          ...
          case 'open-cabang-modal':
            ...
            loadCabangModal({ forceRefresh: true });
            break;
      ...
      async function maybeAutoSyncCabangViewer() {
        if (cabangAutoSyncDone) return null;
        if (cabangAutoSyncPromise) return cabangAutoSyncPromise;

        cabangAutoSyncPromise = (async () => {
          try {
            const res = await fetch('/api/cabang?sync=maukirim', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', 'X-Admin-Token': getOpsToken() },
            });
            const json = await res.json().catch(() => ({}));
            if (!res.ok) throw new Error(json.error || 'Sync cabang gagal.');
            ...
            cabangAutoSyncPromise = null;
          }
        })();

        return cabangAutoSyncPromise;
      }
      ...
      async function loadCabangModal(options = {}) {
        ...
        try {
          ...
          if (!cabangAutoSyncDone) {
            maybeAutoSyncCabangViewer().catch(() => {});
          }
        } catch (err) {
          if (!hasSnapshot) {
      ...
      async function addCabang() {
        ...
        try {
          ...
          else { input.value = ''; document.getElementById('inputArea').value = ''; cabangAutoSyncDone = false; showToast('Cabang berhasil ditambahkan', 'success'); await loadCabangModal({ forceRefresh: true, silent: true }); await refreshAdminViewsAfterWrite({ source: 'admin_cabang_create' }); }
      ...
      async function saveEditCabang(id) {
        ...
        try {
          ...
          else { cabangAutoSyncDone = false; showToast('Cabang diperbarui', 'success'); await loadCabangModal({ forceRefresh: true, silent: true }); await refreshAdminViewsAfterWrite({ source: 'admin_cabang_update' }); }
        } catch { showToast('Terjadi kesalahan jaringan', 'error'); }
      }
      ...
      function renderCabangList() {
        ...
        sorted.forEach(area => {
          ...
          groups[area].forEach(c => {
            ...
            const viewerBadge = c.viewer_login_ready
              ? `<span class="c-pw-badge c-pw-set"><i class="bi bi-shield-check"></i> Viewer siap login</span>`
              : `<span class="c-pw-badge c-pw-empty"><i class="bi bi-arrow-repeat"></i> Menunggu sync Maukirim</span>`;
      ...
      function readAdminCarryFileAsDataUrl(file) {
        if (adminCarryOcrModule && typeof adminCarryOcrModule.readFileAsDataUrl === 'function') {
          return adminCarryOcrModule.readFileAsDataUrl(file);
        }
        ...
        return Promise.reject(new Error('Browser ini belum mendukung pembacaan file untuk OCR.'));
      }
      ...
      async function handleAdminCarryProofChange(event) {
        ...
        try {
          const dataUrl = await readAdminCarryFileAsDataUrl(file);
          if (!dataUrl) {
          ...
          setAdminCarryOcrStatus('error', err && err.message ? err.message : 'Gagal membaca file.');
        }
      }
      ...
      async function fetchJsonWithTimeout(url, options = {}, fallbackTimeoutMs = 15000) {
        const requestLabel = String(options.requestLabel || url || 'Permintaan').trim();
        const fetchOptions = { ...options };
        const timeoutMs = Number(fetchOptions.timeoutMs) > 0 ? Number(fetchOptions.timeoutMs) : fallbackTimeoutMs;
  ...
  const pendingAllocations = buildPendingAllocationItems(pendingData);
      ...
      function renderMismatchMonitorSnapshot(snapshot) {
        ...
        const syncInfo = snapshot && snapshot.ncData && snapshot.ncData.syncInfo ? snapshot.ncData.syncInfo : null;
        document.getElementById('mismatchDateLabel').textContent = getPeriodeLabel(periode) + ' · ' + getAdminSnapshotStateLabel(syncInfo);
