const EMBED_MODE = new URLSearchParams(location.search).get('embed') === '1' || window.self !== window.top;
      ...
      async function addCabang() {
        ...
        try {
          const body = {
            ...
            no_wa: (document.getElementById('inputWa') ? document.getElementById('inputWa').value.trim() : '') || undefined,
            viewer_password: (document.getElementById('inputViewerPw') ? document.getElementById('inputViewerPw').value : '') || undefined,
          };
          ...
          else { input.value = ''; document.getElementById('inputArea').value = ''; if (document.getElementById('inputWa')) document.getElementById('inputWa').value = ''; if (document.getElementById('inputViewerPw')) document.getElementById('inputViewerPw').value = ''; showToast('Cabang berhasil ditambahkan', 'success'); await loadCabangModal({ forceRefresh: true, silent: true }); await refreshAdminViewsAfterWrite({ source: 'admin_cabang_create' }); }
      ...
      function startEditCabang(id) {
        ...
        item.innerHTML = `
          <div class="edit-fields">
            ...
            <input type="tel" id="editWa-${id}" value="${escHtmlAdmin(c.no_wa || '')}" maxlength="20" placeholder="No. WhatsApp (opsional)" inputmode="numeric" style="margin-top:4px">
            <input type="password" id="editViewerPw-${id}" maxlength="100" placeholder="Password Maukirim — kosongkan jika tidak diubah" autocomplete="new-password" style="margin-top:4px">
          </div>
          <div class="c-actions">
      ...
      async function saveEditCabang(id) {
        ...
        const noWaEl = document.getElementById('editWa-' + id);
        const viewerPwEl = document.getElementById('editViewerPw-' + id);
        if (!nama) { showToast('Nama tidak boleh kosong', 'error'); return; }
        const body = { id, nama, area };
        if (noWaEl) body.no_wa = noWaEl.value.trim() || null;
        if (viewerPwEl && viewerPwEl.value) body.viewer_password = viewerPwEl.value;
        try {
      ...
      function renderCabangList() {
        ...
        sorted.forEach(area => {
          ...
          groups[area].forEach(c => {
            const waBadge = c.no_wa
              ? `<span class="c-wa-badge" title="WA: ${escHtmlAdmin(c.no_wa)}"><i class="bi bi-whatsapp"></i></span>`
              : '';
            html += `<div class="cabang-item" id="ci-${c.id}">
              <span class="c-num">${globalIdx++}</span>
              <div class="c-info">
                <div class="c-name">${escHtmlAdmin(c.nama)} ${waBadge}</div>
