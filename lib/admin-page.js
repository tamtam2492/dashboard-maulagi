const EMBED_MODE = new URLSearchParams(location.search).get('embed') === '1' || window.self !== window.top;
      ...
      async function syncMaukirimWa() {
        const btn = document.getElementById('btnSyncMaukirim');
        if (!btn) return;
        btn.disabled = true;
        btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Sync...';
        try {
          const res = await fetch('/api/cabang?sync=maukirim', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Admin-Token': getOpsToken() } });
          const json = await res.json();
          if (!res.ok) {
            showToast(json.error || 'Sync gagal', 'error');
          } else {
            const msg = json.updated > 0
              ? `Sync selesai: ${json.updated} cabang diperbarui no. WA-nya`
              : 'Tidak ada cabang baru yang perlu diupdate (semua sudah terisi atau tidak ditemukan di Maukirim)';
            showToast(msg, json.updated > 0 ? 'success' : 'info');
            if (json.updated > 0) await loadCabangModal({ forceRefresh: true, silent: true });
          }
        } catch { showToast('Terjadi kesalahan jaringan', 'error'); }
        finally { btn.disabled = false; btn.innerHTML = '<i class="bi bi-arrow-repeat"></i> Sync WA Maukirim'; }
      }
