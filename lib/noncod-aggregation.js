(function (root, factory) {
  ...
  function getRekonDifference(ongkir, transfer) {
    ...
    const transferValue = Number(transfer || 0);
    const diff = ongkirValue - transferValue;
    if (diff < 0 && ongkirValue > 0 && Math.abs(diff) <= ADMIN_TRANSFER_FEE_TOLERANCE) return 0;
    return diff;
  }
  ...
  function getAggregatedRekonBaseRows(options) {
    ...
    return [...new Set([...Object.keys(aggregatedNoncod), ...Object.keys(aggregatedTransfer)])].map(cabang => {
      ...
      return {
        ...
        ongkir,
        transfer,
        belum: getRekonDifference(ongkir, transfer),
      };
    });
  }
