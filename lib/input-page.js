...
let snapshotAutoRetryTimer = null;
let snapshotAutoRetryKey = '';
let snapshotAutoRetryAttempt = 0;
const SNAPSHOT_AUTO_RETRY_DELAY_MS = 2500;
const SNAPSHOT_AUTO_RETRY_MAX_ATTEMPTS = 5;
...
function clearSnapshotAutoRetryState() {
  if (snapshotAutoRetryTimer) {
    clearTimeout(snapshotAutoRetryTimer);
    snapshotAutoRetryTimer = null;
  }
  snapshotAutoRetryKey = '';
  snapshotAutoRetryAttempt = 0;
}

function isSnapshotGuardMessage(message) {
  const normalized = String(message || '').trim().toLowerCase();
  return normalized.includes('snapshot noncod periode') && (
    normalized.includes('belum fresh') ||
    normalized.includes('belum siap')
  );
}

function buildSnapshotAutoRetryKey() {
  const vals = getFormValues();
  return [
    inputAsyncState.uploadRevision,
    vals.nama_cabang,
    vals.nominal,
    vals.periode,
    inputAsyncState.proofHash,
    inputAsyncState.transferDatetime,
  ].join('|');
}

function shouldAutoRetrySnapshotContext(context) {
  const nc = context && context.noncodMatch;
  return !!(nc && !nc.match && !nc.splitMatch && !nc.blocked && isSnapshotGuardMessage(nc.message));
}

function getSnapshotGuardDisplayNote(message) {
  const baseMessage = message || 'Snapshot NONCOD belum siap.';
  if (!isSnapshotGuardMessage(baseMessage)) return baseMessage;
  if (snapshotAutoRetryAttempt >= SNAPSHOT_AUTO_RETRY_MAX_ATTEMPTS) return baseMessage;
  return baseMessage + ' Sistem akan coba cek ulang otomatis beberapa detik lagi.';
}

function syncSnapshotAutoRetry(context) {
  if (!shouldAutoRetrySnapshotContext(context)) {
    clearSnapshotAutoRetryState();
    return;
  }

  const retryKey = buildSnapshotAutoRetryKey();
  if (!retryKey) {
    clearSnapshotAutoRetryState();
    return;
  }

  if (snapshotAutoRetryKey !== retryKey) {
    clearSnapshotAutoRetryState();
    snapshotAutoRetryKey = retryKey;
  }

  if (snapshotAutoRetryTimer || snapshotAutoRetryAttempt >= SNAPSHOT_AUTO_RETRY_MAX_ATTEMPTS) {
    return;
  }

  snapshotAutoRetryAttempt += 1;
  snapshotAutoRetryTimer = setTimeout(async () => {
    snapshotAutoRetryTimer = null;
    if (snapshotAutoRetryKey !== retryKey) return;
    if (!canResolveNoncodMatch() || buildSnapshotAutoRetryKey() !== retryKey) {
      clearSnapshotAutoRetryState();
      return;
    }
    await refreshCabangSummary(true);
  }, SNAPSHOT_AUTO_RETRY_DELAY_MS);
}
...
function renderCabangSummary(context) {
  ...
  const scopeLabel = selectedCabang.nama || 'Cabang ini';
  ...
  if (context.proofDuplicate) {
    setCabangSummaryView({
      tone: 'warn',
      badge: 'Bukti Sudah Terpakai',
      meta: selectedCabang.nama + ' · Nominal ' + fmtNom(vals.nominal),
      dateValue: '-',
      ongkirValue: '-',
      diffValue: '-',
      note: 'Bukti transfer ini sudah digunakan.',
    });
    return;
  }
  ...
  if (nc.hold && nc.match) {
  ...
  } else if (isSnapshotGuardMessage(nc.message)) {
    setCabangSummaryView({
      tone: 'pending',
      badge: 'Mencocokkan...',
      meta: scopeLabel + ' · Nominal ' + fmtNom(vals.nominal),
      ...
      ongkirValue: '...',
      diffValue: '...',
      note: 'Sistem sedang menyiapkan pencocokan NONCOD. Coba tunggu beberapa detik lagi.',
    });
  } else if (nc.blocked) {
    ...
    const note = getSnapshotGuardDisplayNote(nc.message || 'Tidak ada data NONCOD yang cocok untuk nominal ini.');
    setCabangSummaryView({
      tone: 'info',
      badge: 'Tidak Cocok',
      meta: scopeLabel + ' · Nominal ' + fmtNom(vals.nominal),
...
async function refreshCabangSummary(force = false) {
  ...
  const summaryKey = [inputAsyncState.uploadRevision, vals.nama_cabang, vals.nominal, vals.periode, inputAsyncState.noncodContextKey, inputAsyncState.proofHash].join('|');
  ...
  try {
    const { res, json } = await fetchJsonWithTimeout('/api/input?dupe=1', {
      ...
      body: JSON.stringify({
        ...
        transfer_datetime: inputAsyncState.transferDatetime || undefined,
        proof_hash: inputAsyncState.proofHash || undefined,
      }),
    ...
    syncSnapshotAutoRetry(lastDupeContext);
...
ocrController = inputOcrModule.createInputOcrController({
  ...
  setTransferDatetime(value) {
    inputAsyncState.transferDatetime = value || '';
  },
...
function blockPendingAppUpdate(btn) {
  if (!window.FrontendOpsReporter || typeof window.FrontendOpsReporter.hasPendingUpdate !== 'function') {
    return false;
  }
  if (!window.FrontendOpsReporter.hasPendingUpdate()) {
    return false;
  }
  if (typeof window.FrontendOpsReporter.promptRefresh === 'function') {
    window.FrontendOpsReporter.promptRefresh();
  }
  alert('Versi aplikasi baru tersedia. Tekan Muat Ulang dulu agar data diproses dengan aturan terbaru.');
  if (btn) {
    btn.innerHTML = '<i class="bi bi-send-fill me-2"></i>Kirim Data';
    btn.disabled = false;
  }
...
async function submitForm() {
  const btn = document.getElementById('btnSubmit');
  if (blockPendingAppUpdate(btn)) return;
  btn.disabled = true;
  ...
  syncSnapshotAutoRetry(lastDupeContext);
    ...
    if (chkJson.proofDuplicate) {
      alert('Bukti transfer ini sudah digunakan.');
      btn.innerHTML = '<i class="bi bi-send-fill me-2"></i>Kirim Data';
      btn.disabled = false;
      return;
    }
    ...
    if (chkRes.ok && chkJson.dupes && chkJson.dupes.length > 0) {
      ...
      const scopeTitle = vals.nama_cabang || (selectedCabang && selectedCabang.nama) || 'Cabang ini';
      detail.innerHTML =
...
async function doSubmit() {
  ...
  try {
    ...
    fd.append('transfer_datetime', inputAsyncState.transferDatetime || '');
